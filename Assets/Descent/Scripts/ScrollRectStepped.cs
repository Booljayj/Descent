...
namespace UnityEngine.UI
{
    ...
    public class ScrollRectStepped : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected virtual void SetContentAnchoredPosition(Vector2 position)
        {
			...
			else position.x = Mathf.Round(position.x / _stepSize) * _stepSize;

            if (!m_Vertical) position.y = m_Content.anchoredPosition.y;
			else position.y = Mathf.Round(position.y / _stepSize) * _stepSize;
        ...
        private void SetNormalizedPosition(float value, int axis)
        {
			...
			newLocalPosition = Mathf.Round(newLocalPosition / _stepSize) * _stepSize;
